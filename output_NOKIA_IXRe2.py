...
#
#
# Politica PORT-QOS (necessaria para porta LAG).
#
configure qos
...
#
#
# Politica VLAN-QOS (nao deve ser utilizada em Router Interface associada a porta LAG).
#
configure qos
...
"""
    ...
    for port in fibra:
        ...
        script += f"""configure port {porta_fo}
connector
...
"""
        ...
        if port["bnm_ativo"]:                
            script +=f"""eth-bn-egress-rate-changes
eth-cfm
...
"""
    ...
    refs = " ".join([f"ref{i+1}" for i in range(len(ptp))])
    script += f"ref-order {refs} ptp gnss"
